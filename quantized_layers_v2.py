...
class Conv2d_ReLU(nn.Module):
    ...
    def forward(self, x, quantization_function, scaling_factor, quantization_bits):
        ...
        out_conv = quantization_function(
            out_conv, scaling_factor, quantization_bits)
...
def update_routing(votes, logits, iterations, bias,
    ...
    for iteration in range(iterations):
        ...
        route = quantization_function(
            route, scaling_factors[iteration*4+0], quantization_bits)
        ...
        preactivate = quantization_function(
            preactivate, scaling_factors[iteration*4+1], quantization_bits_routing)
        activation = squash(preactivate, dim=2)  # bs, co, no, (ho, wo)
        activation = quantization_function(
            activation, scaling_factors[iteration*4+2], quantization_bits)

        act_3d = activation.unsqueeze(1)  # bs, 1, co, no, (ho,wo)
        distances = (votes * act_3d).sum(dim=3)  # bs, ci, co, ho, wo
        logits = logits + distances
        logits = quantization_function(
            logits, scaling_factors[iteration*4+3], quantization_bits_routing)
...
class ConvPixelToCapsules(nn.Module):
    ...
    def forward(self, x, quantization_function, scaling_factors, quantization_bits, quantization_bits_routing):
        ...
        votes = quantization_function(
            votes, scaling_factors[0], quantization_bits)
...
class Conv2DCaps(nn.Module):
    ...
    def forward(self, x, quantization_function, scaling_factors, quantization_bits):
        ...
        output = quantization_function(
            output, scaling_factors[0], quantization_bits)

        output = squash(output, dim=2)
        output = quantization_function(
            output, scaling_factors[1], quantization_bits)
...
class DeepCapsBlock(nn.Module):
    ...
    def forward(self, x, quantization_function, scaling_factors, quantization_bits, quantization_bits_routing=0):
        ...
        """
        x = self.l1(x, quantization_function,
                    scaling_factors[0:2], quantization_bits)
        if self.iterations == 1:
            x_skip = self.l_skip(x, quantization_function,
                                 scaling_factors[6:], quantization_bits)
        else:
            x_skip = self.l_skip(
                x, quantization_function, scaling_factors[6:], quantization_bits, quantization_bits_routing)
        x = self.l2(x, quantization_function,
                    scaling_factors[2:4], quantization_bits)
        x = self.l3(x, quantization_function,
                    scaling_factors[4:6], quantization_bits)
        x = x + x_skip
