...
from quantized_layers_v2 import *
...
class ShallowCapsNet(nn.Module):
    ...
    def __init__(self, input_wh, input_ch, num_classes, dim_output_capsules):
        ...
        self.digit = Capsules(ci=1152, ni=8, co=num_classes,
                              no=dim_output_capsules, iterations=3)
    ...
    def forward(self, x, quantization_function, scaling_factors, quantization_bits, quantization_bits_routing):
        ...
        out_conv = (self.conv(x, quantization_function,
                    scaling_factors[0], quantization_bits[0])).unsqueeze(1)
        ...
        out_primary = out_primary.permute(
            0, 1, 3, 4, 2).contiguous().view(bs, -1, n)

        out_digit = self.digit(out_primary, quantization_function,
                               scaling_factors[6:], quantization_bits[2], quantization_bits_routing[1])
...
class DeepCaps(nn.Module):
    ...
    def __init__(self, input_wh, input_ch, num_classes, dim_output_capsules):
        ...
        self.capsLayer = Capsules(
            ci=t, ni=8, co=num_classes, no=dim_output_capsules, iterations=3)
    ...
    def forward(self, x, quantization_function, scaling_factors, quantization_bits, quantization_bits_routing):
        ...
        # First convolution and conversion to capsules
        l = self.conv1(x, quantization_function,
                       scaling_factors[0], quantization_bits[0])

        l = l.unsqueeze(1)

        # Block One
        l = self.block1(l, quantization_function,
                        scaling_factors[1:9], quantization_bits[1])

        # Block Two
        l = self.block2(l, quantization_function,
                        scaling_factors[9:17], quantization_bits[2])

        # Block Three
        l = self.block3(l, quantization_function,
                        scaling_factors[17:25], quantization_bits[3])
        l1 = l

        # Block Four
        l = self.block4(l, quantization_function,
                        scaling_factors[25:44], quantization_bits[4], quantization_bits_routing[0])
        l2 = l
        ...
        out_caps = self.capsLayer(
            l, quantization_function, scaling_factors[44:57], quantization_bits[5], quantization_bits_routing[1])
