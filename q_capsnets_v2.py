...
def quantized_test(model, num_classes, data_loader, quantization_function, scaling_factors, quantization_bits,
    ...
    for data, target in data_loader:
        ...
        # input quantization
        data = quantization_function(
            data, scaling_factors[0], quantization_bits[0])

        # Output predictions
        output = model(data, quantization_function,
                       scaling_factors[1:], quantization_bits, quantization_bits_routing)
...
def qcapsnets(model, model_parameters, full_precision_filename, num_classes, data_loader, top_accuracy,
    ...
    weights_scale_factors_original = torch.load(
        full_precision_filename[:-3]+'_w_info.pt', map_location=torch.device('cpu'))
    weights_scale_factors = OrderedDict()
    for key, value in weights_scale_factors_original.items():
        weights_scale_factors[key] = torch.min(
            value[0], value[1]*std_multiplier)
    ...
    quantization_function_activations = possible_functions.get(
        quantization_scheme)
    if not quantization_function_activations:
        raise NotImplementedError(
            "Quantization function %s not implemented" % quantization_scheme)
    quantization_function_weights = possible_functions.get(
        quantization_scheme + "_inplace")
    if not quantization_function_weights:
        raise NotImplementedError(
            "Quantization function %s not implemented (inplace version)" % quantization_scheme)
        ...
        for key, value in m:
            if hasattr(value, "leaf") and value.leaf == True:
                ...
                leaf_children, leaf_children_names = get_leaf_modules(
                    top_name + '.' + key, value.named_children(), leaf_children, leaf_children_names)
    ...
    if step1_acc > step1_min_acc:
        # True is the accuracy is higher than the minimum required
        step1_bit_search_sat = [True]
    ...
    step2_weight_bits = [0 for _ in range(len(step1_weight_bits))]
    wgroup_size = math.floor(len(sqnr_tuples) / 3)
    group0_params = 0
    group1_params = 0
    group2_params = 0
    ...
    for i, l in enumerate(sqnr_tuples):
        ...
        elif i >= wgroup_size and i < 2*wgroup_size:
            group1_params += num_params
            step2_weight_bits[l[2]] += 1
        else:
            group2_params += num_params
            step2_weight_bits[l[2]] += 2

    nbits = math.floor((memory_budget_bits - group1_params - 2 *
                       group2_params)/(group0_params+group1_params+group2_params))
    if nbits <= 0:
        print("Error, bits went below 0")
        return "Error"

    step2_weight_bits = [b+nbits for b in step2_weight_bits]
    #### NEW ####
    ...
    if step2_acc >= minimum_accuracy:
        ...
        for l in sqnr_tuples:
            ...
            while step2_acc > minimum_accuracy and step2_weight_bits[l[2]] > 1:
                ...
                step2_acc = quantized_test(model_step2, num_classes, data_loader,
                                           quantization_function_activations, act_scale_factors, step2_act_bits, step2_dr_bits)
                print(step2_weight_bits, step2_act_bits,
                      step2_dr_bits, step2_acc)
            ...
            curr_mem = sum([step2_weight_bits[i]*number_of_weights_inlayers[i]
                           for i in range(len(step2_weight_bits))])
            print(
                f"curr_mem {curr_mem}, memory_budget_bits {memory_budget_bits}")
    ...
    if branchB:   # was not able to satisfy both constraits
        # MODEL MEMORY
        quantized_filename = full_precision_filename[:-
                                                     3] + '_quantized_memory.pt'
        ...
        final_weight_memory_b = sum([number_of_weights_inlayers[i]*model_memory_weight_bits[i]
                                    for i in range(len(number_of_weights_inlayers))])
        ...
        assert len(number_of_weights_inlayers) == len(step2_weight_bits)
        final_weight_memory_b = sum([number_of_weights_inlayers[i]*step2_weight_bits[i]
                                    for i in range(len(number_of_weights_inlayers))])
        wmem_reduction_acc = tot_memory_b / final_weight_memory_b
        print(
            f"Model_accuracy weight mem reduction: {wmem_reduction_acc:.2f}x")
    ...
    layers_dr_position = [pos for pos, val in enumerate(
        dynamic_routing_bits_bool) if val]
    ...
    dr_quantization_pos = [pos for pos, val in enumerate(
        dynamic_routing_quantization) if val]
        ...
        while True:
            if step4a_acc >= step4a_min_acc:
                ...
                for x in range(0, len(dr_quantization_bits)):
                    step4a_dr_bits[dr_quantization_pos[x]
                                   ] = dr_quantization_bits[x]
                ...
                print(step4a_weight_bits, step4a_act_bits,
                      step4a_dr_bits, step4a_acc)
    ...
    final_weight_memory_b = sum([number_of_weights_inlayers[i]*step4a_weight_bits[i]
                                for i in range(len(number_of_weights_inlayers))])
